...
function snapshot(){return [...players.values()].map(p=>({id:p.id,name:p.name,colour:p.colour||'#ff9d65',x:p.x,y:p.y,angle:p.angle,lives:p.lives,score:p.score,active:p.active!==false}));}
...
wss.on('connection',ws=>{
 ...
 const p={id,name:'Player',colour:'#ff9d65',x:400,y:300,angle:-Math.PI/2,lives:3,score:0,active:true,ws};
 players.set(id,p); send(ws,{type:'welcome',id}); playersUpdate();
 ws.on('message',raw=>{
  let m;try{m=JSON.parse(raw.toString())}catch{return}
  if(m.type==='join'||m.type==='restart'){
   p.name=String(m.name||p.name||'Player').slice(0,16);if(typeof m.colour==='string'&&/^#[0-9a-f]{6}$/i.test(m.colour))p.colour=m.colour;
   if(Number.isFinite(m.x))p.x=m.x;if(Number.isFinite(m.y))p.y=m.y;if(Number.isFinite(m.angle))p.angle=m.angle;if(typeof m.colour==='string'&&/^#[0-9a-f]{6}$/i.test(m.colour))p.colour=m.colour;
  ...
  if(m.type==='kill'){
   const victim=players.get(String(m.victimId));
   if(victim&&victim!==p){const transfer=Math.floor(Math.max(0,victim.score)*0.10);victim.score=Math.max(0,victim.score-transfer);p.score=Math.max(0,p.score+transfer);broadcast({type:'kill',killer:id,victim:victim.id,amount:transfer});playersUpdate();}return;
  }
