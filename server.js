...
function snapshot(){return [...players.values()].map(p=>({id:p.id,name:p.name,x:p.x,y:p.y,angle:p.angle,lives:p.lives,score:p.score}));}
...
wss.on('connection',ws=>{
 ...
 const p={id,name:'Player',x:400,y:300,angle:-Math.PI/2,lives:3,score:0,ws};
 ...
 ws.on('message',raw=>{
  ...
  if(m.type==='state'){
   if(Number.isFinite(m.x))p.x=m.x;if(Number.isFinite(m.y))p.y=m.y;if(Number.isFinite(m.angle))p.angle=m.angle;
   if(Number.isFinite(m.lives))p.lives=Math.max(0,Math.floor(m.lives));if(Number.isFinite(m.score))p.score=Math.max(0,Math.floor(m.score));
   playersUpdate();return;
  }
  if(m.type==='hit'){
  ...
  if(m.type==='death'){p.lives=0;broadcast({type:'playerHit',id,lives:0});playersUpdate();return;}
